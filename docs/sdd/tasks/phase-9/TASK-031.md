...
- `parser.ts:120-201` のforEachループ構造がパートごとに独立してmeasureを走査し、`Measure.notes` に順次push（`parser.ts:186-198`）するのみとなっている。そのためMusicXMLの `<duration>`/`<backup>`/`<forward>`/`<chord>` を時刻計算に使っていない。
...
`data-model-v2.md`/`DEC-005`に基づき、`Score`/`Measure`/`Note`型へv2フィールドを追加した。対象は `ticksPerQuarter`, `tempoMap`, `Measure.startTick`, `Note.startTick`/`durationTicks`/`startSeconds`/`durationSeconds`/`voice` である。`parser.ts`は、`<backup>`/`<forward>`/`<chord>`/`<attributes><divisions>`の兄弟要素間の出現順序を保持する必要がある。そのためfast-xml-parser（タグ名ごとに配列化され順序情報が失われる）に加えてDOMParserによる順序保持トラバースを導入した。あわせてパート内カーソル管理・小節頭リセット・chord同時刻・backup/forward巻き戻しを実装した。noteId採番はパート・小節ごとの出現順連番（`noteIndexCounter`）に統一し、`Measure.notes`はstartTick昇順（同tickはpartId→noteIndex順）でソートするようにした。tempoMapは`<direction><sound tempo>`をtick位置付きで収集し、区間積分により`startSeconds`/`durationSeconds`を算出する。
...
- Note型の新規フィールドの正式名称・型（tick単位／ミリ秒単位など）はTASK-030の設計書確定後に判明するため、着手前に当該設計書を必読とする
