...
- [x] 停止中（楽譜未ロードでも）メトロノームをONにすると直ちにクリックが鳴る
- [x] クリックは現在のテンポ設定に追随する（bpm変更で周波数が更新される）
- [x] 拍子に基づく1拍目のアクセントが付く（強調OFF時は全拍同一音量1.0）
- [x] 再生開始で楽譜同期クリックへ切り替わり、一時停止・停止で単独クリックへ戻る
- [x] Transport が独立クロックによって起動されない
- [x] `docs/sdd/requirements/traceability.md` に REQ-006-009 行が追加されている
- [x] `npm run test` / `npm run typecheck` / `npm run lint` / `npm run format:check` / `npm run lint:jp` が全てパスする

  ※ 実起動バイナリでの最終動作確認（単独クリックが実際に鳴るか等）はteam-lead側で
  計測スクリプトを用いて実施予定。ユニット/結線テストとゲートまでが本タスクのスコープ。

## テスト項目

- [x] （新規・結線）有効化で Clock 生成・開始、Transport 非起動
- [x] （新規・ユニット）拍カウンターとアクセント（4拍子・3拍子）
- [x] （新規・結線）play/stop/pause と クロックの切り替え
- [x] （新規・ユニット）setBpm による周波数更新
- [x] （新規・結線）loadScore による拍子反映
- [x] （回帰）楽譜同期モード（TASK-065）の全テスト
