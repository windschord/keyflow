...
- `CLAUDE.md`: 「Electronセキュリティ設定」節（sandbox: true採用・読み取りallowlist・navigation-policy）、
  ...
  ソースコード構成（`src/main/file-handlers.ts`・`src/main/navigation-policy.ts`の追加）を実ファイル確認のうえ更新
