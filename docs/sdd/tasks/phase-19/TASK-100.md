...
| ステータス | DONE（2026-07-13にユーザーが実機確認済み: 言語切り替えの即時反映・再起動後の復元・OSロケール判定） |
| 優先度 | High |
| 見積もり | 60分 |
| 依存タスク | TASK-096〜099 |
...
### ユーザー実機確認の結果（2026-07-13完了）

以下の3項目はタスク完了時点では未実施だったが、2026-07-13にユーザーが
`npm run dev`実起動（PR #45確認時）で確認し、問題なしと判断した。
これをもって本タスクをREVIEWからDONEへ更新した。

1. 設定未保存の初回起動時にOSロケール由来の言語が選択されること（日本語環境で日本語表示を確認）
2. 設定画面での言語切り替えによりヘッダー・設定・About・メニューバーが即時切り替わること
3. アプリ再起動後も選択言語が復元されること
