...
- [ ] Note型に発音時刻（tick/秒）とデュレーションのフィールドが追加されている（TASK-030設計書のフィールド名に準拠）
- [ ] `<chord>` 要素を持つ音符が直前音符と同一startTickになる
- [ ] `<backup>`/`<forward>` を含む譜面でtickが正しく計算される
- [ ] 2パート曲でnoteIdがパート毎連番（`{partId}-M{measureNumber}-N{noteIndex}`）になり、osmd-controller.tsの採番方式と一致する
- [ ] 既存のテストが通る（期待値更新後のテストを含む）
- [ ] 新規テストが追加されている（tick計算・chord同時刻・backup/forward・noteId採番の各ケース）
- [ ] `npm run typecheck` / `npm run lint` がパスする

## テスト項目

- [ ] 単一パート・単一声部の四分音符4つでtickが0,divisions,2*divisions,3*divisionsになる
- [ ] `<chord>` を持つ音符が直前音符とstartTickが等しい
- [ ] `<backup>` 使用後の音符が巻き戻り後のtickから計算される
- [ ] 2パート曲でP1・P2それぞれのnoteIdがN0から連番になる（パート横断連番になっていない）
- [ ] テンポ変化（`sound tempo`）をまたぐ場合にstartSecondsが正しく累積される
- [ ] 休符（`isRest: true`）もtickを消費するが判定対象外として扱われる（型上の確認）
