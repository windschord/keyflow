...
- [x] Cメジャースケール8音（右手）の運指提案が `1-2-3-1-2-3-4-5` になる（統合経路のテストで検証）
- [x] `scale-patterns.ts` が本番経路（`computeFingering` またはworker）からimportされ、デッドコードでなくなっている
- [x] `dp-solver.test.ts:26-39` の弱体化コメント・アサーションが要件由来の期待値に置き換えられている
- [x] スケール非該当の入力ではDPの既存挙動が維持される（回帰テスト）
- [x] `docs/sdd/requirements/traceability.md` の REQ-009-A06 行が更新されている
- [x] 既存のテストが通る
- [x] 新規テストが追加されている（必要な場合）

## テスト項目

- [x] （是正・統合）Cメジャースケール右手上行→ `[1,2,3,1,2,3,4,5]`
- [x] （新規・統合）scale-patternsがサポートする他調・左手の代表ケース1〜2件で定型運指が優先適用される
- [x] （新規・回帰）スケールに該当しない旋律でパターン適用が発動せず、DP結果が従来どおり
- [x] （回帰）空配列・単音・和音などdp-solverの既存エッジケースが全て通る
...
`dp-solver.test.ts:26-39` の「純粋なDPでは1-2-3-1-2-3-4-5とは限らない」という
弱体化コメント・アサーションを削除し、Cメジャースケール右手・Gメジャースケール
左手・Cメジャースケール下降の3ケースで定型運指が厳密に一致することをアサート
する形に是正。加えて非スケール8音入力（`scale-patterns.test.ts`の「非スケール」
ケースと同じ音列）でDPの既存挙動（固定運指と一致しない最適解）が維持される
ことを回帰テストとして追加した。

`npm run test`（32ファイル / 231テスト）・`npm run typecheck`・
`npx eslint`（対象ファイル）はすべてパス。
