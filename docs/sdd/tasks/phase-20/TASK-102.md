...
- [x] 一覧がタイトル・作曲者・最終利用日時を表示する（REQ-017-003）
- [x] 検索で部分一致絞り込み（大文字小文字区別なし）ができる（REQ-017-004）
- [x] 3条件×昇降順の並べ替えができ、既定が最終利用日時の降順（REQ-017-005）
- [x] 削除が確認UIを経由し、ライブラリからのみ削除される（REQ-017-006）
- [x] 空状態の案内とファイルを開く導線が表示される
- [x] 文言がja/en両対応（言語enでの表示テストあり、REQ-017-011）
- [x] 全チェック通過（test/typecheck/lint/lint:jp/format:check）

## テスト項目

- [x] filter純関数: タイトル/作曲者の部分一致・大文字と小文字の非区別・空クエリで全件
- [x] sort純関数: 各キー×昇降順・安定性
- [x] 一覧表示・行クリックでonOpenEntryが呼ばれる
- [x] 削除ボタン→確認→remove呼び出し→一覧から消える
- [x] 空状態表示とonOpenFileDialog呼び出し
- [x] 言語enで見出し・ボタンが英語になる

## 完了サマリー

- 新規ファイル: `LibraryView/index.tsx`、`LibraryView/library-utils.ts`（filter/sort/日時整形の純関数）、両者のテストファイル
- LibraryViewは一覧・検索・並べ替え・削除確認UI・空状態・欠損マーク表示を持つ
- 変更ファイル: `ui-slice.ts`、`ja.ts`、`en.ts`
- ui-sliceには`activeView: 'score' | 'library'`（初期値'library'）と`setActiveView`を追加した
- ja/enには`library`名前空間を追加した
- テスト結果: LibraryView.test.tsx 10件、library-utils.test.ts 14件が新規
- ui-slice.test.tsは追加分を含め39件、全体では880件が通過した
- チェック結果: test/typecheck/lint/lint:jp/format:checkすべて通過した
- コミット: テスト先行が`23af05c`、実装が`ef3681f`
- 未解決事項: App.tsxへの組み込みはTASK-103のスコープとして未着手
- 組み込み対象は表示切り替え・開くフローの結線・欠損検出の3点
