...
test('設定モーダルで言語をEnglishへ切り替えるとヘッダー文言が英語になり、日本語へ戻すと復帰する（TASK-100, REQ-016-003/004）', async () => {
  ...
  await expect(window.getByRole('button', { name: 'ファイルを開く' })).toBeVisible();
  ...
  await expect(window.getByRole('button', { name: 'ファイルを開く' })).toHaveCount(0);
  ...
  // 日本語表示に復帰したことを確認する。
  await expect(window.getByRole('button', { name: 'ファイルを開く' })).toBeVisible();
  await expect(window.getByRole('button', { name: 'Open file' })).toHaveCount(0);
});
