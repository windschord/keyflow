...
- [ ] 楽譜を開いた状態でライブラリ→楽譜へ戻るの往復が即時（再レンダリングなし）で表示される
- [ ] 不可視中に幅0のrenderが実行されない（SVGが破棄されない）
- [ ] ウィンドウリサイズ時の再描画・オーバーレイ再適用の既存挙動に回帰がない
- [ ] 不可視中のズーム変更が可視復帰時に正しく反映される
- [ ] 全チェック通過
...
- [ ] サイズ0通知でrenderスキップ
- [ ] 同一サイズ復帰でrenderスキップ
- [ ] サイズ変更時はrender実行（回帰なし）
- [ ] 不可視中のsetZoom→可視復帰で一度だけrender
- [ ] E2E: 戻った直後にSVGが表示されている
