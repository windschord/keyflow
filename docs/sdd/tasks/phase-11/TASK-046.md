...
- [x] `tests/e2e/app.spec.ts` のカーソル座標検証が、カーソル要素の存在を先にassertし、`if` ガードによる無言スキップが存在しない
- [x] `ScoreRenderer.test` が practiceMode→setPartOpacity の結線（非練習パート0.5／その他1.0）をアサートしている
- [x] `thumbPassingCost` / `fiveOnBlackCost` のテストが設計書のコスト表由来の期待値で存在する
- [x] BPMが元テンポ比20〜200%でクランプされ、実装とテストとUI文言（title）が一致している
- [x] StrictModeレンダリングヘルパーが導入され、主要コンポーネント/フックのテストに適用されている
...
- [x] （是正・E2E）カーソル要素の存在assert＋座標変化assert（無言スキップなし）
- [x] （新規・結線）practiceMode right/left/both それぞれでの `setPartOpacity` 呼び出し内容
- [x] （新規・ユニット）`thumbPassingCost`: 代表ケース（親指くぐり発生/非発生、方向・距離）のコスト
- [x] （新規・ユニット）`fiveOnBlackCost`: 5指が黒鍵に乗るケース/乗らないケースのコスト
- [x] （是正・ユニット）`setBpm`: `originalBpm` 比20%未満・200%超のクランプ、境界値
- [x] （是正・コンポーネント）TempoControl: スライダー/入力の範囲が元テンポ比に連動する
- [x] （新規・環境）StrictModeヘルパー適用下で主要コンポーネント/フックのテストが通る
- [x] （回帰）`npm run test` 全件グリーン、`npm run test:e2e`（実行環境がある場合）、`npm run typecheck` / `npm run lint` パス

## 完了サマリー

- `tests/e2e/app.spec.ts`: カーソル座標検証の2箇所で `if (cursorBoundsBefore && cursorBoundsAfter)` ガードを廃止した。
  先に `#cursorImg-0` の存在を `expect(...).toHaveCount(1)` でassertしてから座標を取得・比較するよう修正した。
- `ScoreRenderer.test.tsx`: `practiceMode`（right/left/both）ごとに `setPartOpacity` が非練習パートへ0.5、
  それ以外へ1.0で呼ばれることをアサートする結線テストを追加した（REQ-002-007）。
- `cost-functions.test.ts`: `thumbPassingCost`（上行時の親指くぐり・下行時の指越え発生/非発生ケース、REQ-009-A03）と
  `fiveOnBlackCost`（黒鍵/白鍵・小指以外のケース、REQ-009-A04）のユニットテストを追加した。実装（`cost-functions.ts`）は
  既存のまま変更していない（期待値の出典をテスト内コメントに明記）。
- BPMクランプ（REQ-006-003）: `ui-slice.ts` の `setBpm` を絶対値20〜400クランプから `originalBpm` 比20%〜200%
  クランプに修正（`get()` を用いて `originalBpm` を参照）。`TempoControl.tsx` の `handleSliderChange` /
  `handleInputBlur` からも冗長な絶対値クランプを除去してstore側のクランプに一本化し、BPM入力の `min`/`max` 属性を
  `originalBpm` 比（20%/200%）から動的に算出するよう変更した。`title` の「20%〜200%」表記と実装の自己矛盾を解消。
  `ui-slice.test.ts` / `Toolbar.test.tsx` を要件由来の期待値（例: originalBpm=100なら20〜200、originalBpm=60なら
  12〜120）へ書き換えた。
- StrictModeヘルパー: `src/renderer/src/tests/test-utils.tsx` に `renderWithStrictMode` /
  `renderHookWithStrictMode` を新設し、`App.test.tsx` / `ScoreRenderer.test.tsx` / `PianoKeyboard.test.tsx` /
  `Toolbar.test.tsx` / `SettingsModal.test.tsx` / `usePractice.test.ts` / `useMidi.test.tsx` の `render`/`renderHook`
  importをこのヘルパーに置き換えて適用した（既存の `render(...)` 呼び出し箇所は変更不要）。
  StrictMode適用で `useMidi.test.tsx` の「should initialize webMidiService on mount」がヘルパー適用前提の
  「マウント→クリーンアップ→再マウント」二重実行により `toHaveBeenCalledTimes(1)` で落ちることが判明した。
  調査の結果、`WebMidiService.initialize()` を2回呼んでも `access` の再代入と `rebindInputs()` の再実行のみで
  副作用が蓄積しない冪等な操作であり、実装側の不具合ではなく呼び出し回数を固定していたテスト側の問題と判断、
  `toHaveBeenCalled()` へ書き換えて解消した（実装は変更していない）。
- `docs/sdd/requirements/traceability.md`: REQ-002-007（△→○）、REQ-006-003（△→○）、REQ-009-A03/A04（×→△、
  DP統合検証は引き続き弱いため○に据え置かず△とした）を更新した。
- `npm run test`（315件全通過）、`npm run typecheck`、`npm run lint` を確認。`npm run test:e2e` も実行し、
  ビルド後Electronを実起動してカーソル座標assertを含む全項目が合格することを確認した（起動プロセスは終了済み）。

## 情報の明確性

### 明示された情報

- 是正対象のfile:line（M6・M8、実コードで検証済み）。内訳: `app.spec.ts:244-248` の `if` ガード、`ui-slice.ts:26` / `TempoControl.tsx:24,36,56,66-67` のクランプ不一致。ほかにcost-functionsテストゼロ、`ScoreRenderer/index.tsx:89-104` の未アサート結線。
- 適用する再発防止策1〜3（分析レポートで承認待ちの方針TASK-046として明記済み）

### 不明/要確認の情報

- なし（すべて確認済み。StrictMode適用で新たな実装バグが露見した場合の扱い（本タスク内修正・切り出しのいずれとするか）は規模により実装時判断とする）
