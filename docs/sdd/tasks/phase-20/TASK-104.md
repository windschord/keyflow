...
| ステータス | DONE（2026-07-13にユーザーが実機確認済み: ライブラリ画面の表示・操作、戻る導線を含め動作問題なし） |
| 優先度 | High |
| 見積もり | 60分 |
| 依存タスク | TASK-101〜103 |
...
以下の2項目はタスク完了時点では未実施だったが、2026-07-13にユーザーが
`npm run dev`実起動（PR #46確認時）で一連の操作を確認し、問題なしと判断した。
これをもって本タスクをREVIEWからDONEへ更新した。

1. ライブラリ画面（検索欄・並べ替えセレクタ・一覧テーブル・削除確認ダイアログ）の
   実際の見た目・レイアウト
2. 欠損ファイル（REQ-017-008）をライブラリから開こうとした際の、ネイティブ
   `alert`ダイアログを含む実際の操作感（TASK-105/106の実機確認と合わせて実施）
