...
- [x] 上記テストケースが全て通過
- [x] 既存のparser.test全件が通過（非回帰）
- [x] `npm run typecheck` / `npm run lint` 通過（`Score` 型利用箇所のコンパイルエラーゼロ。Score生成箇所への `pedalSpans: []` 追加を含む）

## 完了サマリー

`src/renderer/src/types/score.ts` に `PedalSpan` 型と `Score.pedalSpans` を追加した。
`src/renderer/src/lib/musicxml-parser/parser.ts` の `<direction>` 処理に
`<direction-type><pedal type="start|stop|change">` の解析を追加した。
TDD（Red→Green）で実装し、既存の全テスト（40ファイル / 577件）・typecheck・lintが通過することを確認した。
