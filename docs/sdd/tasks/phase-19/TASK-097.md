...
- Header・QuickPanel・MetronomeToggle
- Toolbar配下7コントロール（PlaybackControls / LoopControl / TempoControl /
  PracticeModeSelector / VolumeControl / ZoomControl / FingeringToggle）
- StatsDisplay・FingeringPanel・NoteContextMenu・ScoreRenderer（プレースホルダのみ）

上記の表示文言・aria-label・title属性を`useTranslation()`経由へ置き換えた。
PianoKeyboardはcanvas描画（矢印記号・指番号のみ）で翻訳対象のUI文言が存在しないため
対象外。

`ja.ts`/`en.ts`へ追加した名前空間は次のとおり。

- header / quickPanel / metronome
- playbackControls / loopControl / tempoControl / practiceModeSelector
- volumeControl / zoomControl / fingeringToggle
- statsDisplay / fingeringPanel / noteContextMenu / scoreRenderer
既存の日本語文言を`ja.ts`の値としてそのまま採用したため、既存テスト814件は
無変更のまま全通過した（`beforeEach`へ`language: 'ja'`の明示追加のみ）。
