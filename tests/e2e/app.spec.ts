 ...
 *
 * TASK-100: 本ファイルの各テストは日本語のアクセシブルネームに依存している。
 * E2E起動時は設定未保存のためOSロケール解決（resolveLanguage）となり、非日本語
 * ロケール環境ではテストが決定的にならないため、`e2e-user-data.ts`が用意する
 * 隔離userDataディレクトリへ`ui.language: 'ja'`をシードして起動する。これにより
 * 開発者の実環境のuserData（本来のアプリ設定保存先）を変更することもない。
 */
...
test.beforeEach(async () => {
  // TASK-100: 隔離userDataディレクトリへ ui.language: 'ja' をシードし、OSロケールに
  // 依存せず日本語表示で起動する（開発者の実環境のuserDataは変更しない）。
...
test('アプリ起動→サンプルMusicXML読み込み→再生→手動スクロール→MIDIモック注入で正誤判定・カーソル進行', async () => {
  ...
  // TASK-104: 起動直後は楽譜未読み込みのためライブラリ画面（空状態）が
  // 初期表示される（REQ-017-010、TASK-103でactiveView初期値が'library'へ変更）。
  // ScoreRendererのプレースホルダーは親コンテナがdisplay:noneで不可視になったため、
  // 「楽譜未読み込み」を示すユーザー観測可能な状態としてこちらを検証する。
  ...
  // TASK-053のドロップ案内文、TASK-104のライブラリ空状態ボタンにも
  // 「ファイルを開く」という同一文言が含まれるため、testidでヘッダーのボタンに
  // 一意に特定する。
  ...
  // TASK-104: 楽譜が開かれたことでactiveViewが'score'へ遷移し、ライブラリ画面は
  // アンマウントされる（REQ-017-010の画面切り替え）。
...
test('設定モーダルで言語をEnglishへ切り替えるとヘッダー文言が英語になり、日本語へ戻すと復帰する（TASK-100, REQ-016-003/004）', async () => {
  await expect
    .poll(async () =>
      electronApp.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().length)
    )
  ...
  // 前提: 日本語表示（ui.languageのシード値）で起動していることを先にassertする
  // （2026-07-05再発防止策1: 前提要素の存在を確認せずに結論を出さない）。
  // TASK-104: ライブラリ空状態のボタンも同じ文言「ファイルを開く」を持つため、
  // testidでヘッダーのボタンに絞ったうえでアクセシブルネームを検証する。
  const headerOpenFileButton = window
    .getByTestId('header-open-file-button')
    .and(window.getByRole('button', { name: 'ファイルを開く' }));
  await expect(headerOpenFileButton).toBeVisible();

  // 設定モーダルを開き、言語セレクタが日本語（'ja'）であることを確認する。
  await window.getByRole('button', { name: '設定' }).click();
  const languageSelect = window.locator('#language');
  await expect(languageSelect).toBeVisible();
  await expect(languageSelect).toHaveValue('ja');

  // 言語をEnglishへ切り替える（SettingsModalのlanguageセレクタを実操作）。
  await languageSelect.selectOption('en');
  await expect(languageSelect).toHaveValue('en');

  // 切り替えは即時反映されるため、設定モーダルの閉じるボタンも英語ラベルになる。
  await window.getByRole('button', { name: 'Close' }).click();

  // ヘッダーのボタンのアクセシブルネームが英語へ変わったことを確認する
  // （ユーザー観測可能な結果を合格条件とする）。
  const headerOpenFileButtonEn = window
    .getByTestId('header-open-file-button')
    .and(window.getByRole('button', { name: 'Open file' }));
  await expect(headerOpenFileButtonEn).toBeVisible();
  await expect(headerOpenFileButton).toHaveCount(0);

  // 日本語へ戻す。
  await window.getByRole('button', { name: 'Settings' }).click();
  const languageSelectAgain = window.locator('#language');
  await expect(languageSelectAgain).toBeVisible();
  await expect(languageSelectAgain).toHaveValue('en');
  await languageSelectAgain.selectOption('ja');
  await expect(languageSelectAgain).toHaveValue('ja');
  await window.getByRole('button', { name: '閉じる' }).click();

  // 日本語表示に復帰したことを確認する。
  await expect(headerOpenFileButton).toBeVisible();
  await expect(headerOpenFileButtonEn).toHaveCount(0);
});
