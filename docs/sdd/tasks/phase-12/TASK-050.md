...
- C(5,k) はユニットサイズ k=1..5 で 5/10/10/5/1 通り。6音以上の同時発音（両手混在は TASK-048 のフィルタで排除される前提だが、片手内で5音超の場合）は、ピッチが低い側の5音をDP対象とする。あふれた高音側の構成音には、最も近いピッチの割当済み指（DP対象の最高音の指）を再利用し、固定ペナルティ `OVERFLOW_PENALTY` を加算する（`assignOverflowFingers`）。UIに指番号の欠落を出さない方針であり、根拠を実装コメントで明確化する。
