...
> **改訂（2026-07-07 / Phase 15）**: 本設計は [header.md](header.md)（1行ヘッダー+ポップオーバー、US-012 / [DEC-007](../decisions/DEC-007.md)）に置き換えられる。
> 各コントロールの用途・ツールチップ・無効化条件・対応要件IDの定義は引き続き本ドキュメントを正とし、配置（どこに表示されるか）はheader.mdを正とする。
