...
- [ ] 楽譜を開く前にメトロノームをONにしても、読み込み後のクリック間隔が4分音符
  （480tick）で鳴る（結線テストで検証）
- [ ] `loadScore` 内でPPQ設定がシーケンス再生成より先に行われる（順序テストで検証）
- [ ] メトロノーム無効時は `loadScore` でシーケンスが生成されない
- [ ] 再生成後もクリック発音・一拍目アクセントが機能する
- [ ] `docs/sdd/requirements/traceability.md` の REQ-006-005 行が更新されている
- [ ] `npm run test` / `npm run typecheck` / `npm run lint` が全てパスする

## テスト項目

- [ ] （新規・結線）有効状態の `loadScore` で旧シーケンス `dispose` ＋新シーケンス
  `'4n'` 生成＋ `start(0)`
- [ ] （新規・順序）PPQ設定 → シーケンス再生成の順序検証
- [ ] （新規・ユニット）無効状態の `loadScore` ではシーケンス非生成
- [ ] （新規・回帰）再生成後のクリック発音・アクセント判定
