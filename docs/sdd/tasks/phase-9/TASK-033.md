...
- [x] `App.tsx` の `annotations={[]}` 固定が解消され、鍵盤上に指番号アノテーションが表示される（REQ-005-007）— 本タスク時点では配線のみで描画は未達だったが、フォローアップのTASK-037（`keyboard-renderer.ts` への指番号描画実装）で完了済み。経緯は下記「未完了事項」参照
- [x] 正解率・連続正解数を表示するUIが追加され、日本語ラベルで表示される（US-004、NFR-U-002）
- [x] `npm run dev` での実機起動により、上記6項目すべてが目視で動作確認されている — 本セッションはGUI/ディスプレイのないサンドボックス環境のため実施できなかった。その後、2026-07-05のユーザーフィードバックサイクル（実機での運指表示・視覚フィードバック確認）で担保済み
...
- [x] アノテーションが存在するnoteId対応の鍵盤位置に指番号が表示されることを確認する — 本タスク時点では未実施。TASK-037の実装・テストで確認済み
- [x] 統計表示が `correctNotes`/`incorrectNotes`/`accuracy`/`consecutiveCorrect` の変化に追従して更新されることを確認する
- [x] 実機E2E: `npm run dev` 起動→曲を開く→MIDI（またはクリック）演奏→楽譜色づけ・鍵盤指番号・統計表示・ループ表示を目視確認する — 本タスク時点では未実施（GUI環境なし）。2026-07-05のユーザーフィードバックサイクルの実機確認で担保済み
...
**追記（2026-07-05解消済み）**: 上記フォローアップはTASK-037で実装完了した。鍵盤上の指番号描画（REQ-005-007）は達成済みである。実機目視確認も、同日のユーザーフィードバックサイクル（運指表示の実機確認）で担保された。
