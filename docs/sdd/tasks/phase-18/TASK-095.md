...
### 検証項目5: 悪意入力の拒否がユーザーに表示される経路

TASK-091の各検査（XMLサイズ上限・内部サブセット付きDOCTYPE・zip爆弾）は `MusicXMLParseError` を投げる。
この例外は `src/renderer/src/App.tsx` の `openMusicXmlFile` の `catch`（`console.error` の直後）で捕捉され、
`alert('MusicXML ファイルの解析に失敗しました。ファイル形式を確認してください。')` としてユーザーへ表示される。
拒否条件ごとに `MusicXMLParseError` が送出されることは `parser.test.ts` の各拒否ケースで検証済みであり、
その例外がユーザー向けメッセージに到達する経路は既存のファイルオープン処理と同一である。

なお「実際に悪意ファイルを実UIのファイルダイアログから開いてalertが出る」ところまでの実起動E2Eは未カバーとする。
既存E2Eがダイアログを固定パス（正常なサンプル）へ差し替える構成のためである。
悪意入力の生成と注入はユニットテストで担保済みであり、実起動での最終確認はユーザー操作でも可能なため残件とする。
