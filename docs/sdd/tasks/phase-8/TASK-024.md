...
- `src/renderer/src/App.tsx:85` の `handleOpenFile` は `setScore(parsedScore, filePath, xmlContent)` を呼ぶだけである。その後に `practiceEngine.resetToMeasure(1)` を呼ぶコードがアプリのどこにも存在しない。
...
- 統合テスト `src/renderer/src/tests/integration/practice-flow.test.ts:73` は `engine.resetToMeasure(1)` を**テスト側で手動呼び出し**している。アプリ本体に存在しない初期化をテストが補完してグリーンになっていた（欠落の隠蔽）。
- 付随: 楽譜由来のテンポ（`Score.tempo`、`src/renderer/src/types/score.ts:7`、パーサーは `parser.ts:158-159` で `<sound tempo>` を解析済み）がストアの `originalBpm` / `bpm` に反映されない。そのため120固定のままとなる（`src/renderer/src/store/slices/ui-slice.ts:15-16`）。
...
1. 失敗するテストを先に書く。「`setScore` 後にアプリの初期化経路を通すと `expectedNotes` が空でなくなり、`originalBpm` / `bpm` が楽譜のテンポになる」ことを検証するテストを `practice-flow.test.ts` に追加する。このテストはテスト側で `resetToMeasure` を手動で呼ばない。テスト対象は、App.tsx の `handleOpenFile` 相当のロジックを関数として切り出すか、Testing Library で App コンポーネント経由の検証とする。
