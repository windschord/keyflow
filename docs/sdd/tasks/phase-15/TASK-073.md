...
- [x] 起動直後にグランドピアノ音色のロードが始まり、ロード中の再生要求が待たされる（REQ-013-003）。
      AudioEngineServiceのコンストラクタで既定音色（grand-piano）のロードが即座に開始され
      （TASK-071既存実装）、App.tsxのplaybackAudioEngineラッパーがensurePlaybackVoiceLoaded()
      を待ってからplayAccompanimentを呼ぶことで、ロード中の再生要求は待たされる
