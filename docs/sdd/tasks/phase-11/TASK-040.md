...
- Zustandの `practice-slice.ts` には `errorMode` フィールド（`src/renderer/src/store/slices/practice-slice.ts:6`、初期値 `'wait'` 同 :31）はあるが、**setterアクションが存在しない**。用意されているのは `setPracticeMode` / `setLoopRange` / `toggleLoop` のみである。
- SettingsModalの `updatePracticeSetting`（`src/renderer/src/components/SettingsModal/index.tsx:79-113`）は `defaultErrorMode` をelectron-storeに保存するのみである。storeの `errorMode` へ反映するコードがない。同関数内の `metronomeEnabled` には即時反映コード（同 :94-96、ロールバック :107-109）があり、非対称。
...
- ファイル: `src/renderer/src/components/SettingsModal/index.tsx`
  - 変更内容: `updatePracticeSetting` で `key === 'defaultErrorMode'` の場合に `usePracticeStore.getState().setErrorMode(value)` を即時反映する。`metronomeEnabled` の既存パターン :94-96 と同じ構造とし、保存失敗時のロールバック :107-109 も同様に実装する。
...
- [x] electron-storeへ保存された `practice.defaultErrorMode` がアプリ起動時にstoreへロードされる
...
- `SettingsModal/index.tsx` の `updatePracticeSetting` に `defaultErrorMode` 変更時の即時反映（`usePracticeStore.getState().setErrorMode`）を追加。あわせて保存失敗時のロールバック（`metronomeEnabled` と同一パターン）も追加。
- `App.tsx` の起動時設定ロードeffect（旧 `loadDefaultMetronomeSetting`）を `loadDefaultPracticeSettings` に改名した。あわせて `practice.defaultErrorMode` も読み取って `setErrorMode` でstoreへ反映するよう拡張した。IPC呼び出しは増やさず、既存の `settings.get('practice')` の戻り値を再利用している。
- UIラベルの日本語化については、SettingsModal内の他コントロール（`Enable Metronome by Default` 等）も含め全体が英語表記で統一されていることを確認した。`Default Error Mode` のみを日本語化すると逆に表記が非対称となるため、既存表記のまま維持した（practice-engine本体のロジック変更なし、結線のみのタスク）。
- テスト: `practice-slice.test.ts`（setErrorMode）、`SettingsModal.test.tsx`（即時反映・ロールバック）、`App.test.tsx`（起動時ロード）、`practice-flow.test.tsx` を追加。`practice-flow.test.tsx` はSettingsModal→store→practice-engineの `'pass'` 分岐到達をUI→store→engineの本番経路で検証する。TDDでRed（4件失敗）→実装→Green（`npm run test` 236件全通過）を確認。`npm run typecheck` / `npm run lint` もパス。

## 情報の明確性

### 明示された情報

- 根本原因のfile:line。H2の実コードで検証済み: `practice-slice.ts:6,31` setter欠落、`SettingsModal/index.tsx:79-113` 保存のみ、`App.tsx:96-116` ロード対象外、`practice-engine/index.ts:99` 到達不能分岐。
