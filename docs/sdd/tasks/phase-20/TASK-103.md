...
- [ ] どの経路（ダイアログ・D&D・ライブラリ）で開いてもライブラリへ自動登録される（REQ-017-001）
- [ ] 同一ファイルの再オープンで重複登録されない（REQ-017-002、結線として確認）
- [ ] 起動時（楽譜未読み込み）にライブラリ画面が表示される（REQ-017-010）
- [ ] ライブラリの行クリックで楽譜が開き、楽譜画面へ遷移する（REQ-017-007）
- [ ] 欠損ファイルはエラー通知+欠損表示+削除の選択肢が出る（REQ-017-008）
- [ ] Headerからライブラリ画面へいつでも戻れる
- [ ] 全チェック通過

## テスト項目

- [ ] openFile成功→library.upsert呼び出し（title/composer/path、結線テスト）
- [ ] 起動時にLibraryViewが表示される（score未読み込み）
- [ ] onOpenEntry成功→読み込みフロー実行→activeView='score'
- [ ] onOpenEntry not-found→トースト+削除確認の分岐
- [ ] Headerのライブラリボタンで画面が切り替わる
