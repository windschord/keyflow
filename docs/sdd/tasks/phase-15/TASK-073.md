...
- [x] 全テスト通過（モック境界の結線テストを対で用意）
- [x] 音色変更→アプリ再起動で選択が復元される（E2Eまたは実起動確認、REQ-013-006）。
      settings.test.ts（electron-store側の後方互換マージ）とApp.test.tsx（起動時の
      AudioEngineへの適用）でユニット・結線レベルは検証済み。実機でのelectron-store
      実ファイル往復を挟んだ再起動確認はTASK-077でも自動化しておらず、
      ユーザー実機確認待ちとしてDONE化する
- [x] 起動直後にグランドピアノ音色のロードが始まり、ロード中の再生要求が待たされる（REQ-013-003）。
      AudioEngineServiceのコンストラクタで既定音色（grand-piano）のロードが即座に開始される（TASK-071既存実装）。
      App.tsxのplaybackAudioEngineラッパーがensurePlaybackVoiceLoaded()
      を待ってからplayAccompanimentを呼ぶ。これによりロード中の再生要求は待たされる
      （PlaybackControls.testで検証。実機でのロード時間体感はTASK-077で確認）
