...
| ID | タスク名 | ステータス |
| ---- | --------- | ----------- |
| TASK-024 | [BugFix] スコア読み込み後の練習セッション初期化 | TODO |
| TASK-025 | [BugFix] 楽譜スクロールのCSSレイアウト修正 | TODO |
| TASK-026 | [BugFix] 曲の再生/停止機能の暫定実装 | TODO |
| TASK-027 | [BugFix] テンポ・メトロノーム・効果音の結線 | TODO |
| TASK-028 | [BugFix] Toolbar UXの全面改善（日本語ラベル・機能整理） | TODO |
| TASK-029 | 要件定義追加: US-010 曲の再生（お手本演奏） | TODO |
| TASK-030 | 設計: 時刻ベースデータモデルへの再設計 | TODO |
| TASK-031 | パーサーの時刻付与・noteId統一実装 | TODO |
| TASK-032 | practice-engineの両手・和音同時判定対応 | TODO |
| TASK-033 | 楽譜上の視覚フィードバック実装（osmd-controller空実装の解消） | TODO |
| TASK-034 | 実起動E2Eテストの導入（Playwright for Electron） | TODO |
| TASK-035 | macOSパッケージングの追加 | TODO |
| TASK-036 | ドキュメントの実態同期とQA運用是正 | TODO |
