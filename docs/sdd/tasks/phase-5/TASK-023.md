# TASK-023: 楽譜の自動スクロール・カーソル追従実装（OSMD連携）
...
- [x] `currentNoteId` が変化すると、対応する音符へカーソルが移動する。
- [x] 練習進行に伴い、現在演奏中の小節が常に画面内に収まるよう自動スクロールする（`REQ-002-003`）。
- [x] 楽譜が表示領域より大きい場合、ユーザーが手動でもスクロールできる。
- [x] 既存の zoom 変更・パートハイライトの挙動を壊さない。
- [x] 追加した挙動に対する単体テストが通る。
