...
| US-018 | Windowsの配布形態拡充（arm64対応・インストール不要版） | 中 | 一部廃止 | [詳細](stories/US-018.md) @stories/US-018.md |
| US-019 | WindowsのMicrosoft Store配布（SAC対策） | 高 | 定義済 | [詳細](stories/US-019.md) @stories/US-019.md |
...
| REQ-018 | Windows配布形態の拡充（x64/arm64・NSIS/portable・完全性検証対象） | US-018 | 一部廃止（NSIS/portable配布はREQ-019へ移行） |
| REQ-019 | WindowsのMicrosoft Store配布（MSIX生成・x64/arm64・.exe配布廃止） | US-019 | 定義済 |
...
│   ├── US-018.md              # Windowsの配布形態拡充（arm64・portable。一部廃止）
│   └── US-019.md              # WindowsのMicrosoft Store配布（SAC対策）
└── nfr/
    ├── performance.md          # 性能要件
