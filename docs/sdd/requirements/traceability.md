...
| REQ-016-005 | ○ | App.test（起動時にsettings:get('ui').languageとresolveLanguageの結果をui-slice.languageへ反映）+ locale.test（Main起動時のメニュー言語解決）+ 実機確認（再起動後の言語復元、2026-07-13ユーザー確認済み） |
...
| REQ-017-008 | ○ | App.test（onOpenEntry not-found→トースト表示+削除確認ダイアログの分岐）で結線を検証済み。欠損ファイルを開いた際のalert・削除確認の操作感は2026-07-13にユーザーが実機確認済み（ネイティブダイアログを伴う実バイナリE2Eは未整備のまま） |
