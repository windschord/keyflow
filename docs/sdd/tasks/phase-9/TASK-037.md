...
- [x] 現在の判定グループのノーツに指番号アノテーションがある場合、対応する鍵に指番号が描画される（白鍵・黒鍵両方をユニットテストで検証）
- [x] アノテーションのないノーツでは描画されない（fingerNumberなし・別noteIdのケースも検証）
- [x] 既存のテストが通る（PianoKeyboard.test.tsx 9件全パス。全体スイートはTASK-038完了時の検証に含まれる）
- [x] 新規テストが追加されている（指番号描画テスト5件）

## テスト項目

- [x] 指番号付きノーツで fillText 相当の描画が呼ばれる（ユニット）
- [ ] 実機で指番号が視認できる（手動E2E）※実行環境でGUI目視確認が不可のため未チェック。運指提案実行後に鍵盤ガイド上へ数字が表示されることをユーザー確認推奨

## 完了サマリー（2026-07-05）

- `keyboard-renderer.ts` の `drawKey` に指番号描画を追加。現在の判定グループ（expectedNotes）のノーツに `fingerNumber` 付きアノテーションがある場合のみ、鍵の下部中央に描画（白鍵: 黒字13px、黒鍵: 白字11px、bold）
- 実施の前提として、ユーザーグローバル設定の deny ルール `Read(**/*key*)` が PianoKeyboard パスに誤ヒットしていた問題をユーザー承認のうえ修正（実際の鍵ファイルパターンに置換）
