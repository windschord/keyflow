...
- [ ] 実起動確認: ペダル付きMusicXMLを再生してエラーなく完了し、停止時に音が残留しない
      （TASK-077の実機QAへ送る。本タスクではユニット/結線テストでイベント長・
      releaseAll呼び出しを担保済み）
...
`stopAccompaniment` / `pauseAccompaniment` に `accompanimentSynth.releaseAll()` を追加し、
`setLoopPoints` ではループ折り返し（loopEnd tick）で `releaseAll()` をスケジュールする
Transport.scheduleを登録、スコア差し替え・ループ再設定時に旧スケジュールをclearする実装とした。
...
- 実機での聴覚確認（ペダル付きMusicXML再生時の音長の聴感）はTASK-077（実機QA）へ送る
